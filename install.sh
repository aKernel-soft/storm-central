#!/bin/bash
# STOLER universal installer – Termux & Linux
set -e
...
install_deps() {
    if $IS_TERMUX; then
        ...
        case "$PKG" in
            apt-get) sudo apt-get update && sudo apt-get install -y curl jq transmission-cli git;;
            pacman) sudo pacman -Syu --noconfirm curl jq transmission-cli git;;
            dnf) sudo dnf install -y curl jq transmission-cli git;;
            zypper) sudo zypper install -y curl jq transmission-cli git;;
            apk) doas apk add curl jq transmission-cli git;;
            xbps-install) sudo xbps-install -Sy curl jq transmission git;;
            emerge) sudo emerge -av curl jq transmission git;;
            *) echo "[!] Unknown package manager. Please install: curl jq transmission-cli git"; return 1;;
        esac
    fi
}
